...
const handleUpdateRole = (user) => {
  ...
  return (
    <div>
        <div>
            <h2 className="md:text-5xl text-3xl font-Cinzel font-semibold text-center mt-16">All Users </h2>
        </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {users.map((user, index) => (
                  <tr key={user._id} className="hover:bg-gray-100">
                    ...
                    <td className=" ">
                     {
                        user.role === 'admin' ? 'Admin' :    <button onClick={() => handleUpdateRole(user)} className='bg-[#6feccb] p-4 rounded-lg'>{user.role}</button>
                     }
                        </td>
                    <td className="px-6 py-4">
